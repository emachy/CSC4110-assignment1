...
import 'package:projectname/controllers/localTempDB.dart';
import 'package:image_cropper/image_cropper.dart';

import 'const.dart';

// Author: Birol
// This file deals with the functionality concerning loading and uploading a profile picture.  This file also creates random uids
// and gets a timestamp for the user.

class Utils {
  static Widget loadingCircle(bool isLoading) {
    // This function creates a loading Widget just in case some information needs to load.
    return isLoading
        ? Positioned(
            child: Container(
              child: Center(
  ...
  static Future<File> cropImageFile(File image) async {
    // This function crops a profile picture.
  ...
  static Widget cacheNetworkImageWithEvent(
      // This function gives an error just in case something goes wrong with uploading a photo.
      context,
      String imageURL,
      double width,
      double height) {
  ...
  static Future<MyProfileData> updateLikeCount(
      // This function updates the like count of the poem and of comments.
    ...
    List<String> newLikeList = await LocalTempDB.saveLikeList(
        // The likes that the user makes are stored locally instead of on
        // Firebase.
    ...
    MyProfileData myNewProfileData = MyProfileData(
        // Grabs the user's profile information in order to like.
    ...
    updateMyData(
        myNewProfileData); // Updates the user's profile information concerning likes.
    isThread // Updates the like count on Firebase
  ...
  static List<DocumentSnapshot> sortDocumentsByComment(
      // Pretty self-explanatory.  Comments are sorted by age.
  ...
  static String commentWithoutReplyUser(String commentString) {
    // This deals with the content of comments that are not reply comments.
  ...
  static String readTimestamp(int timestamp) {
    // Grabs the timestamps of comments/poems
  ...
  static String getRandomString(
          int
              length) => // Creates random strings for poem uid purposes.  Gives each poem a unique identifier.
